...
// conditional rendering only when search certain news
  ...
  return (
    <div className="md:container md:mx-auto">
        <div>
        <h1 className="text-center mb-10 font-extrabold leading-none tracking-tight text-gray-900 text-4xl lg:text-5xl dark:text-white">
         {newsPagesTitle} 
        </h1>
        </div>
        <div className="grid  grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
          {(Array.isArray(setOfNews)?setOfNews:setOfNews.results).map((News, idx) => (
            <div
              key={idx}
